...
J <- 100 #total number of jobs parallelized (should match array=1-J in the command line)
P <- R/J #number of simulations that THIS job will do.

# filestem <- paste0("/Users/reederh/Dropbox/Harrison Reeder/Paper2_AFT/Code/ROSMAP_application_revision/")
# scripts <- filestem
# output <- "/Users/reederh/Documents/Projects/AFT/sims/" #place to store big files potentially
# stanpath <- paste0(scripts,"Stan/")

filestem <- paste0("/n/home05/hreeder/paper2/20221027/")
scripts <- paste0(filestem,"scripts/")
output <- paste0(filestem,"output/")
stanpath <- paste0(scripts,"stan/")
...
source(paste0(scripts,"sim_functions_2022-09-21.R"))
source(paste0(scripts,"AFT_functions_spline_2022-10-27.R"))
source(paste0(scripts,"BayesSurv_AFTtvstan_2022-10-27.R"))
...
begin <- Sys.time()
stan_AFT_tvcov_piece_compiled <- stan_model(paste0(stanpath,"AFT_all_tvcov_piece.stan"),verbose = TRUE)
end <- Sys.time()
print("parametric pw aft compilation time:")
print(end-begin)

begin <- Sys.time()
stan_AFT_tvcov_spline_compiled <- stan_model(paste0(stanpath,"AFT_all_tvcov_spline.stan"),verbose = TRUE)
end <- Sys.time()
print("parametric spline aft compilation time:")
print(end-begin)
...
for(r in 1:P){
    ...
    for(model_base in c("lognormal",
      ...
      for(model_type in c("invariant","piecewise","spline")){
        ...
        test_fit <- BayesSurv_AFTtvstan(Y = Y_temp,
                                        ...
                                        Xtv = x_bin_temp,
                                        baseline = model_base, tbp = FALSE, #for parametric models, no tbp
                                        J = 5, dirichlet_alpha_fixed = FALSE, dirichlet_alpha_data = 1,
                                        a_dirichlet_alpha = 1, b_dirichlet_alpha = 1,
                                        prior_centering = "none", #for parametric models, no centering prior
                                        ...
                                        init=init_func_temp,
                                        seed=1234, control = control_temp)
        ...
        #save final iteration location for initializing tbp prior model
        beta_tv_simstemp <- 
          do.call(cbind,test_fit$stan_fit@sim$samples[[1]][paste0("beta_tv[",1:nP_tv_temp,"]")])
        colnames(beta_tv_simstemp) <- NULL
        out_list_temp[["final_iter"]] <- list(
          intercept=tail(test_fit$stan_fit@sim$samples[[1]][["intercept"]],n=1),
          scale=tail(test_fit$stan_fit@sim$samples[[1]][["scale"]],n=1),
          beta=c(
            tail(test_fit$stan_fit@sim$samples[[1]][["beta[1]"]],n=1),
            tail(test_fit$stan_fit@sim$samples[[1]][["beta[2]"]],n=1)),
          beta_tv=beta_tv_simstemp[NROW(beta_tv_simstemp),]
        )
        
        # #I was gonna compute loocv and then delete log_lik samples to save storage
        # #oh god except this breaks other functions that use stan_fit, abort!
        # # test_fit$stan_fit@sim$samples[[1]][paste0("log_lik[",1:N,"]")] <- NULL
        # saveRDS(test_fit, file = paste0(output,"fit_sim_",iter_seed,
        #                                 "_n_",N,
        ...
        # matplot(x=t_seq_temp,
        #         y=cbind(true_basehaz,cond_basehaz_temp[,1:2]),
        #         type="l",col=c("black",rep("blue",3)),lty=c(1,1,3,3),
        #         main=paste0("dgm: ",dgm_base_label,"_",dgm_type_label,", model: ",
        #                     model_base_label,"_",model_type_label),
        #         sub=paste0("n = ",N))
        ...
        # matplot(x=t_seq_temp,
        #         y=cbind(true_basesurv,cond_basesurv_temp),
        #         type="l",col=c("black",rep("blue",3)),lty=c(1,1,3,3),
        #         main=paste0("dgm: ",dgm_base_label,"_",dgm_type_label,", model: ",
        #                     model_base_label,"_",model_type_label),
        #         sub=paste0("n = ",N))
        ...
        cond_af_temp <- 
          AF.AFTtvstan_tvcov(stan_fit = test_fit, p_seq = p_seq_cond_temp,
              type = "conditional", thin=1,verbose=FALSE)
        ...
        # cbind(median=cond_af_temp[,1],mean=cond_af_temp[,4],
        #   lb_quant=cond_af_temp[,2],lb_asymp=cond_af_temp[,4] - 1.96*cond_af_temp[,5],
        #   ub_quant=cond_af_temp[,3],ub_asymp=cond_af_temp[,4] + 1.96*cond_af_temp[,5])
  ...
  if (P==r){print("We did it fam")}
  # if ((difftime(end_time_r, begin_time_1,units="hours")/r)>(time_h/P)){
  #   write.csv(matrix(data=0,nrow=1,ncol=1),paste0(flagpath,"_",as.character(iter_seed),".csv"))
  #   print(paste("This job may be in danger of not finishing on time, so it is being flagged."))
  #   print(paste("There are only ",round(time_h-avg_runtime*r/60,3),"hours allocated remaining."))
  # }
