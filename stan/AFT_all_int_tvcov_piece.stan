...
data {
  ...
  matrix[N_i,nP_tv_piece] basis_piece_iL; //matrix of nP_tv basis functions evaluated at yL
  matrix[N_i,nP_tv_piece] basis_piece_iU; //matrix of nP_tv basis functions evaluated at yU
  int L_ind_i[N_i];

  // data for left truncated observations
  int<lower=0> N_l;
...
transformed data{
  ...
  for (l in 1:N_i) {
    ...
    cut_cat_iL[l] = firstgreater(tv_time_iL[l], knots_tv);
    cut_cat_iU[l] = firstgreater(tv_time_iU[l], knots_tv);
  }
...
transformed parameters{
  real<lower=0> scale; // Weibull Shape    
  ...
  //generate the integrated value used in the likelihood for right censored obs
  for (j in 1:N_m) {
    if(cut_cat_m[j] == 0){
  ...
  //generate the integrated value used in the likelihood for interval censored obs
  for (l in 1:N_i) {
    if(cut_cat_iL[l] == 0){
      Vout_iL[l] = base_time_i[l];
    } else {
      Vout_iL[l] = base_time_i[l] + basis_piece_iL[l] * exp(-Xtv_i[l]*beta_tv);
    }

    if(cut_cat_iU[l] == 0){
      Vout_iU[l] = base_time_i[l];
    } else {
      Vout_iU[l] = base_time_i[l] + basis_piece_iU[l] * exp(-Xtv_i[l]*beta_tv);
    }
  }
